...
2. Call `get_agent_capabilities` for the selected Agent and collect only missing required inputs. Follow the selected workflow's `inputSchema` exactly; do not copy the downstream service's MCP arguments into the Agent input.
3. Call `invoke_agent` with a stable idempotency key. For an ordinary natural-language request, preserve the user's original wording in `input.message`; `workflowId` may be omitted when AIOS can select one unambiguous workflow.
