# Lazyoffice AIOS Agent Builder Universal Plugin

這個套件同時支援 ChatGPT／Codex Universal Plugin 與 Claude Plugin。它只安裝客戶端設定：

1. `build-aios-agent` Skill，讓模型用動態 Grill-me 方式協助建立 AI 員工。
2. HTTPS Remote MCP Connector：`https://aios-mcp.lazyoffice.app/mcp`。
3. 在 Claude Code／Cowork 中，以相容的 `SessionStart`、`UserPromptSubmit`、`PreToolUse`、`PermissionRequest`、`PostToolUse` 與 `Stop` command Hooks 建立完整同步閉環。
...
- 若以 ChatGPT Developer mode 建立個人 Plugin，請註冊 `https://aios-mcp.lazyoffice.app/mcp`，完成 AIOS OAuth 登入後啟用 `build-aios-agent` Skill。

## Claude Chat 的限制

Claude Chat 的自訂 Skill ZIP 不等於 Connector。若使用的 Claude 介面不支援 Plugin，請另在 Claude 的 Connectors 設定加入上面的 Remote MCP URL，再安裝獨立的 `build-aios-agent.skill.zip`。此情況沒有 Claude Code 同等的生命週期 Hooks，Skill 會在每輪回覆前主動呼叫 MCP 同步。
