...
const label = 'app.lazyoffice.aios-server';
...
<dict>
  ...
  <key>StandardOutPath</key><string>${xml(path.join(logs, 'lazyoffice-aios-server.stdout.log'))}</string>
  <key>StandardErrorPath</key><string>${xml(path.join(logs, 'lazyoffice-aios-server.stderr.log'))}</string>
</dict>
</plist>
`;
...
for (let attempt = 0; attempt < 40; attempt += 1) {
  try {
    const response = await fetch('http://127.0.0.1:8700/api/health');
    if (response.ok) {
      healthy = true;
      break;
    }
...
if (!healthy) throw new Error('AIOS server LaunchAgent started but health did not become ready');

console.log(`Installed and started ${label}`);
console.log('AIOS server health verified: http://127.0.0.1:8700/api/health');
