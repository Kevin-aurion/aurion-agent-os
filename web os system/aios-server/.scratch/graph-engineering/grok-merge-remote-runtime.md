...
1. Preserve the local clean-room rename: the product/plugin/marketplace identity is **LazyOffice / lazyoffice-aios-builder**. Do not restore tracked `aurion-aios-builder` plugin, installer, release, or marketplace paths.
...
3. Integrate all four remote commits' functionality:
   ...
   - account isolation.
4. Port the remote `use-aios-agent` skill/plugin additions into the **LazyOffice** plugin source and all generated one-click/marketplace release structures. Update manifests, docs, package scripts/checksums consistently. Do not keep Aurion-branded duplicate artifacts.
...
7. For generated zip/checksum release files, use the repository packaging script after sources are merged; do not choose an arbitrary conflicted binary. Ensure all published artifacts are LazyOffice-branded and include both build and use skills.
