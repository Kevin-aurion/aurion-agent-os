...
正式對外的 `aurion-aios.lazyoffice.app` 不應依賴某個終端視窗裡的 `npm run dev`。主機以 LaunchAgent `app.aurion.aios-server` 執行建置後的 `dist/index.js`，`KeepAlive` + `RunAtLoad`，log 在 `~/Library/Logs/aurion-aios-server.*.log`；更新後重跑 `npm run install:host-service`。
