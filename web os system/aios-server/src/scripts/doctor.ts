...
async function main() {
  ...
  // Memory (Phase 1): Qdrant vector store on loopback.
  if (!config.memory.enabled) {
    line(true, 'Qdrant reachable', 'skipped (MEMORY_ENABLED=false)');
  } else {
    try {
      const qUrl = new URL(config.memory.qdrantUrl);
      ...
      line(false, 'Qdrant reachable', `invalid QDRANT_URL: ${config.memory.qdrantUrl}`);
    }
  }
