...
| `AIOS_MCP_PROFILE` | `full` | `builder` exposes only the twenty-one Agent Builder/Stop-guard/Agent Runtime tools; local customer installation uses this least-privilege profile |
...
- Unauthenticated MCP requests return OAuth protected-resource metadata. Only the twenty-one Builder/Runtime tools are exposed publicly; approval, Skill confirmation, rollback and activation are absent.
...
The installer pre-approves only the fifteen non-runtime `mcp__aios__...` builder synchronization tools in Claude Code so background capture does not stall on a permission dialog. The six Runtime tools are intentionally not pre-approved because invoking an Agent can have side effects. Existing ask/deny rules and all unrelated tool permissions are preserved.
