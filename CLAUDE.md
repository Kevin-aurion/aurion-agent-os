# lazyoffice — 專案總覽（AIOS 本地代理工作站）

> 🔄 **現況交接文件**：本輪（2026-07）大量新增的架構、資料模型、端點、治理模型與已知限制，
> 統一記錄在根目錄 [`AGENTS.md`](AGENTS.md)（給 Codex／協同 AI）。**開發前請先讀它**，本檔保留專案總覽與開發流程規範。


此目錄是 Kevin（Lazyoffice Group / 奧聯集團）的工作根目錄。本輪（2026-07 起）主線專案是 **AIOS — 本地優先多代理「AI 員工」作業系統**。
