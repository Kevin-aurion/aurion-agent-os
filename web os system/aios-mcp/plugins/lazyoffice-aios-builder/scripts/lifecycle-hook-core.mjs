...
const LAZYOFFICE_SERVER_RE = '(?:plugin_lazyoffice-aios-builder_lazyoffice_aios|claude_ai_lazyoffice_aios(?:_2)?)';
const LIFECYCLE_TOOL_RE = new RegExp(`^mcp__${LAZYOFFICE_SERVER_RE}__(start_agent_build|prepare_agent_build_prompt|guard_agent_build_stop)$`, 'u');
const DRAFT_SYNC_TOOL_RE = new RegExp(`^mcp__${LAZYOFFICE_SERVER_RE}__(sync_agent_build_turn|sync_agent_build_artifact|upsert_agent_build_snapshot)$`, 'u');
...
function promptContext(state) {
  ...
  if (state.startRequired && !state.startSynced) {
    calls.push(
      'First call the connected Lazyoffice AIOS MCP tool `start_agent_build` with the exact current user prompt as `initialRequest`, '
  ...
  return hookContext('UserPromptSubmit', [
    'Lazyoffice AIOS lifecycle gate: this is an Agent or Skill build/training turn.',
    ...calls,
...
function stopContext(state) {
  ...
  return hookContext('Stop', [
    'Lazyoffice AIOS has not yet confirmed the complete lifecycle for this Agent-building turn.',
    ...calls,
      ...
      return {
        ...
        output: {
          hookSpecificOutput: {
            ...
            permissionDecisionReason: 'Lazyoffice AIOS inert lifecycle or draft synchronization for the active matching session.',
          },
        },
      };
    }
